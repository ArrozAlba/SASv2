<?php
	...
	function uf_print_encabezado_pagina($as_titulo,$as_numsol,$ad_fecregsol,&$io_pdf)
	{
		...
		$io_pdf->addText(37,124,6,"ELABORADO POR"); // Agregar el título
		$io_pdf->addText(27,118,6,"ANALISTA DE COMPRAS"); // Agregar el título
		$io_pdf->addText(37,63,6,"FIRMA / SELLO"); // Agregar el título
		$io_pdf->addText(130,122,6,"VERIFICADO POR"); // Agregar el título
		$io_pdf->addText(120,63,6,"FIRMA / SELLO / FECHA"); // Agregar el título
		$io_pdf->addText(228,122,6,"AUTORIZADO POR"); // Agregar el título
		$io_pdf->addText(210,63,6,"ADMINISTRACIÓN Y FINANZAS"); // Agregar el título
		$io_pdf->addText(322,122,6,"ANALISTA DE PRESUPUESTO"); // Agregar el título
		$io_pdf->addText(337,63,7,"FIRMA /  FECHA"); // Agregar el título
		$io_pdf->addText(435,122,6,"OFICINA DE PLANIFICACION Y PRESUPUESTO"); // Agregar el título
	...
	function uf_print_detalle_cuentas($la_data,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
				         ...
				         'outerLineThickness'=>0.5,
						 'innerLineThickness' =>0.5,
						 'cols'=>array('codestpro'=>array('justification'=>'center','width'=>235), // Justificación y ancho de la columna
						 			   'cuenta'=>array('justification'=>'center','width'=>46), // Justificación y ancho de la columna
									   'denominacion'=>array('justification'=>'left','width'=>209), // Justificación y ancho de la columna
					...
					if($lb_valido)
					{
						...
						if($lb_valido)
						{
							...
							for($li_s=1;$li_s<=$li_totrowcuentas;$li_s++)
							{
								...
								$ls_denestpro3  = trim($io_report->ds_cuentas->data["denestpro3"][$li_s]);
								$ls_spgcuenta   = trim($io_report->ds_cuentas->data["spg_cuenta"][$li_s]);
								$ls_denominacion= trim($io_report->ds_cuentas->data["denominacion"][$li_s]);
								if($ls_estmodest==1)
								{
									$ls_codestpro=$ls_codestpro1."-".$ls_codestpro2."-".$ls_codestpro3;
									$ls_denestpro3."  -  ".$ls_codestpro;//$ls_codestpro=$ls_codestpro1.$ls_codestpro2.$ls_codestpro3;
								}
								else
								{
									$ls_codestpro=$ls_codestpro1."-".$ls_codestpro2."-".$ls_codestpro3;
									$ls_denestpro3."  -  ".$ls_codestpro;//$ls_codestpro=$ls_codestpro1." - ".$ls_codestpro2." - ".$ls_codestpro3." - ".$ls_codestpro4." - ".$ls_codestpro5;
								}
								
								$li_montocta=$io_report->ds_cuentas->data["monto"][$li_s];
								$li_montocta=number_format($li_montocta,2,",",".");
								$la_data[$li_s]=array('codestpro'=>$ls_denestpro3."  -  ".$ls_codestpro,'cuenta'=>$ls_spgcuenta,'denominacion'=>$ls_denominacion,'monto'=>$li_montocta);
							}	
