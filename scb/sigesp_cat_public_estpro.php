	 ...
	 if (($ls_estpreing=='1')&&($ls_tipo=="ingreso"))
	 {
	 	$ls_sql = "SELECT DISTINCT(spg_ep3.codestpro3) as codestpro3,
			     ...
			     WHERE spg_ep1.codemp='".$la_empresa["codemp"]."'
				   ...
				   AND spg_ep1.codestpro1||spg_ep2.codestpro2||spg_ep3.codestpro3||spg_ep3.estcla IN (SELECT SUBSTR(codintper,1,75)||SUBSTR(codintper,126,1) FROM sss_permisos_internos WHERE codusu='".$_SESSION["la_logusr"]."' UNION SELECT SUBSTR(codintper,1,75)||SUBSTR(codintper,126,1) FROM sss_permisos_internos_grupo,sss_usuarios_en_grupos WHERE codusu='".$_SESSION["la_logusr"]."' AND sss_permisos_internos_grupo.codgru=sss_usuarios_en_grupos.codgru)
