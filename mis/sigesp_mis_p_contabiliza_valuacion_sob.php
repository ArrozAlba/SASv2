		...
		case "PROCESAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					$lb_valido=$in_class->uf_procesar_contabilizacion_contrato($ls_codcon,$ls_codval,$ls_fecha,$ld_fechacontaasig,$la_seguridad);
					if($lb_valido)
					{
...
function uf_verdetalle(codval,codcon)
{
	...
	window.open("sigesp_mis_pdt_sobval.php?codval="+codval+"&codcon="+codcon+"","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=500,height=400,left="+Xpos+",top="+Ypos+",location=no,resizable=no");
}
</script>
