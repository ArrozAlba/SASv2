...
<html>
<head>
<title>Definici&oacute;n del Tipo de Uso</title>
...
<?Php
	...
	$ls_sistema="SFC";
	$ls_ventanas="sigesp_sfc_d_tipo_uso.php";
...
/************************************************************************************************************************/
    ...
    require_once("class_folder/sigesp_sfc_c_tipo_uso.php");
	...
	$io_tipo = new sigesp_sfc_c_tipo_uso();
...
/************************************************************************************************************************/
	...
	if($ls_operacion=="ue_nuevo")
	{
		...
		$io_seq->uf_ver_secuencia(sfc_tipouso_id_tipouso_seq,$ls_prox);
...
/************************************************************************************************************************/
	...
	/***********************  verificar si posee "Usos" ***************************************************************/
	     $ls_sql="SELECT *
                   FROM sfc_uso
                  WHERE id_tipouso='".$ls_codtipo."'";
		...
		if($rs_datauni==false)
		{
			...
			$is_msg="Error en uf_select_tipouso ".$io_funcion->uf_convertirmsg($io_sql->message);
		}
		else
		{
			if($row=$io_sql->fetch_row($rs_datauni))
			{
		        ...
		        $is_msg->message ("El Tipo de Uso tiene un Renglón asociado no se puede eliminar!!!");
...
/************************************************************************************************************************/
	elseif($ls_operacion=="ue_validar")
	{
	    $ls_sql="SELECT *
                   FROM sfc_tipouso
                  WHERE dentipouso ilike '".$ls_nomtipo."'";
		//print $ls_sql;
	    $lb_valido=$io_utilidad->uf_datacombo($ls_sql,&$la_tipo_rubro);

		if ($lb_valido==true)
		{
		  $is_msg->message ("El Tipo de Uso esta registrado!!");
		  $io_datastore->data=$la_tipo_rubro;
		  $ls_codtipo=$io_datastore->getValue("id_tipouso",1);
		  $ls_nomtipo=$io_datastore->getValue("dentipouso",1);
		}
		else{
      ...
      <tr>
        <td width="516" height="100"><div align="center">
            <table  border="0" cellspacing="0" cellpadding="0" class="formato-blanco" align="center">
              <tr>
                <td colspan="2" class="titulo-ventana">Definici&oacute;n del Tipo de Uso </td>
              </tr>
              <tr>
                <td>
...
function ue_nuevo()
{
			...
			f.txtcodtipo.value="";
			f.txtnomtipo.value="";
			f.action="sigesp_sfc_d_tipo_uso.php";
			f.submit();
			}
			else
			{
