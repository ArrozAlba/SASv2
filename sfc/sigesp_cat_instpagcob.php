	 ...
	 <table width="560"  border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
       <tr>
         <td >
           ...
           <input name="txtdescta" type="hidden" id="txtdescta" value="<?php print $ls_descta ?>">
           <input name="txtctascg" type="hidden" id="txtctascg" value="<?php print $ls_ctascg ?>">
         ...
         <select name="comboforma" size="1" id="comboforma"  onChange="actualizar_pagina();">
           ...
           <?Php
					for($li_i=1;$li_i<=$li_totalfilas;$li_i++)
					{
						 ...
						 if ( ($ls_codigof!='04' and $ls_codigof!='06' and $ls_codigof!='08') and $ls_estatus!='true')
						 {
       ...
       if($ls_metforpago=='R'){ ?>
       	<tr>
         <td width="196" height="30" align="right">No. Factura</td>
         <td colspan="2" >
         	<input name="txtnumfac" type="text" id="txtnumfac" style="text-align:center " value="<?php print $ls_numfac ?>" size="28" maxlength="25"  >
         </td>
        </tr>
       <?php }else{ ?>
       		<input name="txtnumfac" type="hidden" id="txtnumfac" style="text-align:center " value="<?php print $ls_numfac ?>"   >
       <?php }?>
       <tr>
       ...
       <tr>
         ...
         <td colspan="2"><label>
         	...
         	<?php
		 	     ...
		 	     if($ls_codforpag != '02'){
		 	     	$filtro= " WHERE codban IN (SELECT codban FROM scb_ctabanco WHERE codban!='---') ";
		 	     }else{
		 	     	$filtro=" WHERE codban!='---' ";
		 	     }

		 	$ls_sql="SELECT codemp, codban, nomban FROM scb_banco ".$filtro.
		 		" ORDER BY nomban ASC";
	    ...
	    //---------------  combo banco="seleccion..." si forma pago='C'-->efectivo,B-->banco  -------------
			   if($ls_metforpago=='C' or $ls_metforpago=='R')
			   {
			     print "<script language=JavaScript>document.form1.txtmetforpago.value='".$ls_metforpago."' </script>";
	        ?>
              <select name="combo_banco" size="1" id="combo_banco"  onChange="actualizar_cta();">
       ...
       <?php
       if($ls_codforpag == '05' or $ls_codforpag == '07'){ ?>
       	<tr>
         <td height="31"><div align="right">Cta. Banco</div></td>
         <td colspan="2">
         	<input name="txtctaban" type="text" id="txtctaban"  style="text-align:left" value="<?php print $ls_ctabanco ?>" size="17" maxlength="17" readonly="true">
         	<a href="javascript:ue_cat_ctabanco();"><img src="../shared/imagebank/tools15/buscar.gif" width="15" height="15" border="0" alt="Cat&aacute;logo Cuentas"></a>
         </td>
       </tr>
       <?php }else{ ?> 
       	<input name="txtctaban" type="hidden" id="txtctaban"  style="text-align:left" value="<?php print $ls_ctabanco ?>">
       <?php }?>
      ...
      $ld_monB=$io_funsob->uf_convertir_cadenanumero($ls_montot);
	   ...
	   /*if(($ls_metforpago=="C"))
		...
		<script language="JavaScript">
		 ...
		 var ls_numfac = '<?php print $ls_numfac ?>' ;
		 ...
		 <?php
		 if($ls_codforpag == '05' or $ls_codforpag == '07'){ ?>
		 	var ls_ctaban = document.form1.txtctaban.value;
		 <?php }?>
		 ...
		 opener.ue_cargarfpago(ls_numinst,ls_numfac,ld_monto,ls_denforpago,ls_nombanco,ls_codban,ls_codforpag,ls_fecins,ls_obsins,ls_metforpag,ls_ctaban);
		 close();
		</script>
		<?PHP
}
...
function ue_cat_ctabanco(){
	f=document.form1;
	ls_banco = f.combo_banco.options[f.combo_banco.selectedIndex].value;
	if(ls_banco == ""){
		alert("Debe seleccionar un Banco!!")
	}else{
		pagina="sigesp_cat_ctabanco.php?codban="+ls_banco;
		popupWin(pagina,"catalogo ctabanco",650,450);
	}

}

function actualizar_cta(){
	f=document.form1;
	ls_tippago = f.comboforma.options[f.comboforma.selectedIndex].value;
	if( ls_tippago=="05" || ls_tippago=="07"){
		f.txtctaban.value = "-------------------------";
	}
}
...
function ue_cargarnota(nomcli,codcli,numnot,dennot,tipnot,fecnot,monto,estnot)
{
	  ...
	  f.operacion.value="";
	  f.action="sigesp_cat_instpagcob.php";
	  f.submit();
}
	...
	with(f)
	{
		...
		if (ue_valida_null(txtmonto,"Monto")==false)
		...
		else if (txtnuminst.value!="000000000000000" && txtmetforpago.value!="D")
		 {
			 ...
			 else if(combo_banco.value=="" && txtmetforpago.value!="R")
			 {
			 	alert("Debe seleccionar un banco.");
			 }else if(txtctaban.value=="-------------------------" && ( (combo_banco.value=="05") || (combo_banco.value=="07") ) ){
			 	alert("Seleccione la Cuenta Banco Asociada!!");
			 }
			 else if(txtnumfac.value=="" && txtmetforpago.value=="R"){
			 	alert("Introduzca el número de Factura objeto de Retención!");
			 }
			 else
			 {
			   operacion.value="ENVIAR_DATOS";
			   action="sigesp_cat_instpagcob.php";
			   submit();
			 }
		 }
