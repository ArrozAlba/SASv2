...
<script type="text/javascript" language="javascript1.5" src="js/stm31.js"></script>
...
<table width="780" border="0" align="center" cellpadding="0" cellspacing="0" class="contorno" >
  ...
  <tr>
    <td width="503" height="20" class="cd-menu"><span class="descripcion_sistema Estilo1">Sistema de Facturaci&oacute;n</span></td>
    <td width="275" class="cd-menu"><div align="right"><b><?PHP print date("j/n/Y")." - ".date("h:i a");?></b></div></td>
  </tr>
  <tr>
...
/************************************************************************************************************************/
	...
	*************************************************************************************************************************/
	...
	$ls_titulorubrosagri="Rubros Agricolas";
	...
	if(array_key_exists("operacion",$_POST))
	{
		...
		$ls_tipocanal     =$_POST["cmbtipocanal"];
		if($ls_tipocanal==0)
		{
			$lb_sel_public="selected";$lb_sel_privado="";						
		}
		else
		{
			$lb_sel_public="";$lb_sel_privado="selected";			
		}
		...
		$ls_codtipcli     = $_POST["cmbtipocliente"];
		$ls_hidstatuscliente=$_POST["hidstatuscliente"];
...
/************************************************************************************************************************/
	else
	{
		...
		$ls_tipocanal     =0;
		$lb_sel_public="selected";
		$lb_sel_privado="";
		$ls_codtipcli     = "";
		$ls_hidstatuscliente="t";	
        if($ls_clicot!="")
         {
           $ls_codcli=$ls_clicot;
...
/************************************************************************************************************************/
	...
	{
				   	if($ls_hidstatus=="")//Entra si no proviene de uin registro existente
				   	{				   
					...
					if ($ls_productor=="NO")
					{
							$lb_valido=$io_cliente->uf_guardar_cliente($ls_codcli,$ls_tipcli.$ls_cedcli,$ls_nomcli,$ls_dircli,$ls_telcli,$ls_celcli,
														$ls_codpai,$ls_codest,$ls_codmun,$ls_codpar,$ls_precioestandar,$ls_productor,$la_seguridad,$ls_codtipcli,$ls_tipocanal,$ls_hidstatus);
							$ls_cadena= " SELECT * ".
										" FROM  sfc_cliente ".
	...
	/***********************  verificar si posee "RUBROS" ***************************************************************/
		...
		else
		{
			if($row=$io_sql->fetch_row($rs_datauni))
			{
		        ...
		        $is_msg->message ("El Cliente tiene Rubros Agricolas asociados, no se puede eliminar!!!");
			}
			else
			{
...
/************************************************************************************************************************/
	elseif($ls_operacion=="ue_validar_carta")
	{
		...
		if ($lb_valido==true)
		{
		    $is_msg->message ("El Numero de Documento ya existe, esta registrada para otro Productor!!");
			$ls_nrocartagr="";
	    }
	}
/*****************************************************************************************************************************/
	...
	elseif($ls_operacion=="ue_actualizar_check")
	{
	   $ls_sql="SELECT *
		...
		if ($ls_nrocartagr!="")
		{
			...
			$lb_valido=$io_cliente->uf_guardar_cliente($ls_codcli,$ls_tipcli.$ls_cedcli,$ls_nomcli,$ls_dircli,$ls_telcli,$ls_celcli,
	                                                   $ls_codpai,$ls_codest,$ls_codmun,$ls_codpar,$ls_precioestandar,$ls_productor,$la_seguridad,$ls_hidstatus);
		}
	}
}
/*****************************************************************************************************************************/
...
elseif($ls_operacion=="ue_cargarcliente")
	{
	  $ls_sql="SELECT *
		...
		if ($lb_valido==true)
		{
		  ...
		  $ls_productor=$io_datastore->getValue("productor",1);
		  $ls_hidstatuscliente=$io_datastore->getValue("estatus",1);
		  if ($ls_productor=="SI")
		  {
			$ls_codemp=$la_datemp["codemp"];
			$ls_cadena2="SELECT *
...
/*********************************************************      RUBROS AGRoCOLAS           ********************************************************************************/
			 $ls_cadena1="SELECT ra.*,cla.denominacion as dencla,cla.prod_estimada,ru.denominacion as denrubro,
						   ...
						   WHERE ra.id_clasificacion=cla.id_clasificacion AND ra.cod_clasificacion=cla.cod_clasificacion 
						     AND cla.id_rubro=ru.id_rubro AND ru.id_renglon=re.id_renglon AND ci.id_ciclo=ru.id_ciclo 
						     AND CAST(re.id_tipoexplotacion as integer)=1 AND ra.codcli='".$ls_codcli."'";
			$arr_rubrosagri=$io_sql->select($ls_cadena1);
			if($arr_rubrosagri==false&&($io_sql->message!=""))
			{
...
/*********************************************************      RUBROS PECUARIOS           ********************************************************************************/
			$ls_cadena2="SELECT rp.*,cla.denominacion as dencla,cla.prod_estimada,cla.animal_has,
						 ...
						 FROM   sfc_rubropec_cliente rp,sfc_clasificacionrubro cla,sfc_rubro as ru,sfc_renglon re
						 WHERE  rp.id_clasificacion=cla.id_clasificacion AND rp.cod_clasificacion=cla.cod_clasificacion
						 AND 	cla.id_rubro=ru.id_rubro
						 AND    ru.id_renglon=re.id_renglon
						 AND    CAST(re.id_tipoexplotacion as integer) =2
						 AND    rp.codcli = '".$ls_codcli."'";
			$arr_rubrospec=$io_sql->select($ls_cadena2);
			if($arr_rubrospec==false&&($io_sql->message!=""))
			{
				$is_msg->message("No hay registros de rubros pecuarios");
            ...
            <table  border="0" cellspacing="0" cellpadding="0" class="formato-blanco" align="center">
              ...
              <tr>
                <td >
				  <div align="right">
				    ...
				    <input name="hidsta"    		 type="hidden" id="hidsta"             value="<?php print $ls_hidsta?>">
				    ...
				    <input name="hidcodpar" 	     type="hidden" id="hidcodpar"          value="<?php print $ls_codpar?>">
				    ...
				    <input name="removerretenciones" type="hidden" id="removerretenciones" value="<?php print $li_removerretenciones?>">
				    <input name="filasrubrosagri" 	 type="hidden" id="filasrubrosagri"    value="<?php print $li_filasrubrosagri?>">
				    <input name="removerrubrosagri"  type="hidden" id="removerrubrosagri"  value="<?php print $li_removerrubrosagri?>">
				    <input name="filasrubrospec"     type="hidden" id="filasrubrospec"     value="<?php print $li_filasrubrospec?>">
				    <input name="removerrubrospec"   type="hidden" id="removerrubrospec"   value="<?php print $li_removerrubrospec?>">
				    <input name="hidcodcli"          type="hidden" id="hidcodcli"          value="<?php print $ls_codcli?>">
				    <input name="hidcedcli"          type="hidden" id="hidcedcli"          value="<?php print $ls_tipcli?>">
				    <input name="hidreadonly"        type="hidden" id="hidreadonly"        value="<?php print $ls_readonly?>">
				    <input name="hidreadonlyced"     type="hidden" id="hidreadonlyced"     value="<?php print $ls_readonlyced?>">
				    <input name="hidstatuscliente"   type="hidden" id="hidstatuscliente"   value="<?php print $ls_hidstatuscliente;?>">
		          </div>              </td>
			  <td class="titulo-conect"><div align="left">Estatus: 
		        <?php if($ls_hidstatuscliente==t){print "Activo";}else{print "Inactivo";}?>			  
		        </div></td>
			  </tr>
			  <tr>
              ...
              <tr>
				...
				<input name="txtnomcli" type="text" id="txtnomcli"  onKeyPress="return(validaCajas(this,'x',event,50))"  value="<? print  $ls_nomcli?>" size="50" maxlength="225" >
                  <a href="javascript:ue_abrir('sigesp_sss_p_repassword_admin.php');"></a></td>
              </tr>
              ...
              <tr align="left">
                ...
                <td><input name="txtcelcli" id="txtcelcli"   value="<? print $ls_celcli?>" type="text" size="20" maxlength="20" onKeyPress="return(validaCajas(this,'t',event,254))"></td>
              </tr>
                           <tr>
             ...
             <tr>
			  ...
			  <tr>
                    <td height="13" align="right">Tipo de Canal </td>
                    <td height="22" ><select name="cmbtipocanal" id="cmbtipocanal">
                      <option value="0" <?php print $lb_sel_public;?>>PUBLICO</option>
                      <option value="1" <?php print $lb_sel_privado;?>>PRIVADO</option>
                      </select>
                      Clasificaci&oacute;n<span class="style6">
                      <?php  
					  	$ls_sql="SELECT codtipcli,dentipcli,obstipcli ".
                                "   FROM sfc_tipocliente ".
								"   ORDER BY codtipcli ASC";
				        $lb_valtipocliente=$io_utilidad->uf_datacombo($ls_sql,&$la_tipocliente);
					 	if($lb_valtipocliente)
						{
							$io_datastore->data=$la_tipocliente;
							$li_filas=$io_datastore->getRowCount("codtipcli");
						}
						else{$li_filas=0;}
					 ?>
                      <select name="cmbtipocliente" size="1" id="select">
                        <option value="" <?php if($ls_codtipcli==""){ print "selected";};?>>Seleccione...</option>
                        <?php
						for($li_i=1;$li_i<=$li_filas;$li_i++)
						{
							 $ls_tipocliente=$io_datastore->getValue("codtipcli",$li_i);
							 $ls_dentipcli=$io_datastore->getValue("dentipcli",$li_i);
							 $ls_obstipcli=$io_datastore->getValue("obstipcli",$li_i);
							 if ($ls_tipocliente==$ls_codtipcli)
							 {
								  print "<option value='$ls_tipocliente' selected title='$ls_obstipcli'>$ls_dentipcli</option>";
							 }
							 else
							 {
								  print "<option value='$ls_tipocliente' title='$ls_obstipcli'>$ls_dentipcli</option>";
							 }
						}
	                ?>
                      </select>
			...
			?>
             ...
             <tr>
               <td height="19" colspan="4" align="right">
			   	<div align="center">
                <input name="botptoscol" type="button" id="botptoscol" value="Puntos de Colocaci&oacute;n" onClick="javascript:ue_cargar_ptos_colocacion();">                
               	</div>			   </td>
              </tr>
    </table>
        </form>
...
function ue_guardar()
{
	...
	{*/
	ls_estatus=f.hidstatuscliente.value;
	if(ls_estatus=='t')
	{	
		li_incluir=1;
		li_cambiar=1;
    ...
    {
         alert("El cliente se encuentra en estatus inactivo, no puede editar el registro");
    }
}
...
function ue_buscar()
{
    ...
    if(li_leer==1)
	{
	pagina="sigesp_cat_cliente.php?desde=MAESTRO";
	popupWin(pagina,"catalogo",600,450);
	}
	else
	{
		alert("No tiene permiso para realizar esta operacion");
...
function ue_cargarcliente(codcli,cedcli,nomcli,dircli,telcli,celcli,codpai,codest,codmun,codpar,productor,precioestandar,tentierra,estatus,codtipcli,tipocanal)
{
	f=document.form1;
	f.hidstatus.value="C";
	...
	f.hidstatuscliente.value=estatus;
	f.cmbtipocliente.value=codtipcli;
	if(tipocanal=='t')
	{
		f.cmbtipocanal.value=1;
	}
	else
	{
		f.cmbtipocanal.value=0;
	}
	if (productor=="V")
	 {
...
function ue_catrubrosagri()
{
	...
	if(ls_estatus=='t')
	{
		...
		f.cmbprecioestandar.value!="" && f.txtnrocartagr.value!="" && f.txtnrohect.value!="")
		{
			...
			popupWin(pagina,"catalogo",550,450);
		}
		else
		{
			if (f.txtcedcli.value=="")
			...
			alert("Debe indicar el Numero de Hectareas que posee el Productor");
			txtnrohect.focus();
			}
		}
	}
...
function ue_abrir(pagina)
{
	...
	if( (f.txtcedcli.value != "") && (f.txtnomcli.value == "") && (f.operacion.value == "ue_validar") ){
		popupWin(pagina,"catalogo",600,500);
	}
...
function ue_cargar_ptos_colocacion()
{
	f=document.form1;
	ls_estatuscliente=f.hidstatuscliente.value;
	ls_estatus_registro=f.hidstatus.value;
	if(ls_estatus_registro=='C')
	{
		if(ls_estatuscliente=='t')
		{
			if(f.txtcedcli.value!="" && f.txtnomcli.value!="" && f.txtdircli.value!="" && f.txttelcli.value!="" && f.cmbestado.value!="" && f.cmbmunicipio.value!="" && f.cmbparroquia.value!="" && f.cmbprecioestandar.value!="" )
			{
				popupWin("sigesp_cat_puntos_colocacion.php?codcli="+f.hidcodcli.value+"&cedcli="+f.cmbtipcli.value+f.txtcedcli.value+"&nomcli="+f.txtnomcli.value,"catalogo",600,560);
			}
			else
			{
				if (f.txtcedcli.value=="")
				...
				alert("Debe indicar una Parroquia");
				cmbparroquia.focus();
				}else if(f.txtproductor.value=="SI")
				{
					alert("entro");
					if (f.txtnrocartagr.value=="")
					{
						alert("Debe especificar un No de Carta Agraria");
						txtnrocartagr.focus();
					}
	...
	{
		 alert("Debe guardar el cliente para poder registrar sus puntos de colocación");
	}
