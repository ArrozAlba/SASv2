...
<html>
<head>
<title>Cat&aacute;logo de Rubros Pecuarios</title>
  ...
  <p align="center">
    <input name="operacion" type="hidden" id="operacion"  value="<? print $ls_operacion?>">
  	 ...
  	 <table width="500" border="0" align="center" cellpadding="1" cellspacing="1">
    	<tr>
     	 	<td width="496" colspan="2" class="titulo-celda">Cat&aacute;logo de Rubros Pecuarios</td>
    	</tr>
	 </table>
	 <br>
...
{
			$ls_cadena="SELECT cla.id_clasificacion,cla.cod_clasificacion,cla.denominacion as dencla,cla.animal_has,cla.prod_estimada,cla.id_rubro,
						r.denominacion as denrubro,r.id_renglon,re.denominacion as denrenglon,re.id_tipoexplotacion from
						sfc_clasificacionrubro cla, sfc_rubro r,sfc_renglon re where cla.id_rubro=r.id_rubro AND 
						r.id_renglon=re.id_renglon AND CAST(re.id_tipoexplotacion as integer)='2' AND cla.denominacion ilike '".$ls_denrubro."'";
			...
			else
			{
				if($row=$io_sql->fetch_row($rs_datauni))
				{
					...
					for($z=1;$z<=$totrow;$z++)
					{
						...
						$codclapec=$io_data->getValue("cod_clasificacion",$z);
						...
						$has=$funsob->uf_convertir_numerocadena($io_data->getValue("animal_has",$z));
						//$prod_est=$io_data->getValue("prod_estimada",$z);
						$prod_est=$funsob->uf_convertir_numerocadena($io_data->getValue("prod_estimada",$z));
						print "<td><a href=\"javascript: aceptar('$codclapec','$codrubro','$nomrubro','$cod_tipo','$cod_tipoprod','$tipo_prod','$descripcion','$prod_est','$has');\">".$codrubro."</a></td>";
...
<script language="JavaScript">
  function aceptar(codclapec,codrubro,nomrubro,cod_tipo,cod_tipoprod,tipo_prod,descripcion,prod_est,has)
  {  	
    opener.ue_cargar_rubropec(codrubro,nomrubro,cod_tipo,cod_tipoprod,tipo_prod,descripcion,prod_est,has,codclapec);
	close();
  }
  ...
  function ue_search()
  {
	  ...
	  f.action="sigesp_cat_rubropec.php";
	  f.submit();
  }
