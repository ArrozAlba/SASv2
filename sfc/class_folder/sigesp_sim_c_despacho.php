...
class sigesp_sim_c_despacho
{
	...
	function uf_sim_obtener_dt_solicitud($as_codemp,$as_numsol,&$ai_totrows,&$ao_object)
	{
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->despacho MÉTODO->uf_sim_obtener_dt_solicitud_I ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$ai_totrows=0;
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				{
					$lb_valido=false;
					$this->io_msg->message("CLASE->despacho MÉTODO->uf_sim_obtener_dt_solicitud_II ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				}
				else
				{
...
//				"   AND sep_dt_articulos.codart=sim_dt_despacho.codart".
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->despacho MÉTODO->uf_sim_obtener_dt_pendiente_I ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$ai_totrows=0;
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				...
				if(($ls_numorddesaux=="")||($ls_numorddes==$ls_numorddesaux))
				{
					...
					{
						$lb_valido=false;
						$this->io_msg->message("CLASE->despacho MÉTODO->uf_sim_obtener_dt_pendiente_II ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
					}
					else
					{
						$ai_totrows=$ai_totrows+1;
	...
	function uf_sim_select_despacho($as_codemp,$as_numorddes)
	{
		...
		if($li_exec===false)
		{
			$this->io_msg->message("CLASE->Despacho MÉTODO->uf_sim_select_despacho ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_sim_insert_despacho($as_codemp,&$as_numorddes,$as_numsol,$as_coduniadm,$ad_fecdes,$as_obsdes,$as_codusu,
								    $as_estdes,$as_estrevdes,$as_codunides,$ls_codtie,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql="INSERT INTO sim_despacho (codemp, numorddes, numsol, coduniadm,fecdes, obsdes, codusu, estdes, estrevdes, codunides,codtiend)".
				"     VALUES ('".$as_codemp."','".$as_numorddes."','".$as_numsol."','".$as_coduniadm."','".$ad_fecdes."',".
				"             '".$as_obsdes."','".$as_codusu."','".$as_estdes."','".$as_estrevdes."','".$as_codunides."','".$ls_codtie."')";
		//print $ls_sql;
		$li_row=$this->io_sql->execute($ls_sql);
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->Despacho MÉTODO->uf_sim_insert_despacho ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
				...
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="INSERT";
				$ls_descripcion ="Insertó el registro de la orden de Despacho ".$as_numorddes.
								 " Para la Unidad Administrativa ".$as_coduniadm." Asociada a la Empresa ".$as_codemp." de la Tienda ".$ls_codtie." ".$ls_nomtie;
	...
	function uf_sim_insert_dt_despacho($as_codemp,$as_numorddes,$as_codart,$as_numreg,$as_codalm,$as_unidad,$ai_canorisolsep,
									   $ai_canart,$ai_preuniart,$ai_monsubart,$ai_montotart,$ai_orden,$ai_canpenart,$as_codproveedor,$ls_codtie,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if ($lb_valido)
		{
		$ls_sql="INSERT INTO sim_dt_despacho(codemp,numorddes,codart,numreg,codalm,unidad,canorisolsep,canart,preuniart,".
		        "                            monsubart,montotart,canpenart,orden,codtiend,cod_pro)".
				"     VALUES ('".$as_codemp."','".$as_numorddes."','".$as_codart."','".$as_numreg."','".$as_codalm."','".$as_unidad."','".$ai_canorisolsep."',".
				"             '".$ai_canart."','".$ai_preuniart."','".$ai_monsubart."','".$ai_montotart."','".$ai_canpenart."','".$ai_orden."','".$ls_codtie."','".$as_codproveedor."')";


		$li_row=$this->io_sql->execute($ls_sql);
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->Despacho MÉTODO->uf_sim_insert_dt_despacho ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
				/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="INSERT";
				$ls_descripcion ="Insertó el detalle de la orden de Despacho ".$as_numorddes." del articulo ".$as_codart.
								 " Para la Unidad Administrativa ".$as_coduniadm." Asociada a la Empresa ".$as_codemp." de la Tienda ".$ls_codtie." ".$ls_nomtie;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
	...
	function uf_sim_insert_dt_scg($as_codemp,$as_codart,$as_codcmp,$ad_feccmp,$as_sccuenta,$as_debhab,$ai_monto,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->Despacho MÉTODO->uf_sim_insert_dt_scg ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_select_movimiento($ls_metodo,&$rs_metodo,$as_codart,$as_codalm,$as_codproveedor,$ls_codtie)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_sim_procesar_dt_despacho($as_codemp,$as_numorddes,$as_codart,$as_codalm,$as_unidad,$ai_canorisolsep,$ai_canart,$ai_preuniart,$ai_monsubart,$ai_montotart,$ai_orden,$as_nummov,$ad_fecdesaux,$as_numsol,$ai_canpenart,$as_codproveedor,$ls_codtie,$aa_seguridad,$as_codprodoc)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//print $ai_canart;
		if ($lb_valido)
		{
			...
			$lb_valido=$this->uf_select_movimiento($ls_metodo,&$rs_metodo,$as_codart,$as_codalm,$as_codproveedor,$ls_codtie);
			if($lb_valido)
			{
					...
					while(($row=$this->io_sql->fetch_row($rs_metodo))&&(!$lb_break))
					{
						...
						if($this->ls_gestor=="MYSQL")
						{
							$ls_sql="SELECT SUM(CASE opeinv WHEN 'ENT' THEN candesart ELSE -candesart END) total".
									...
									"   AND numdocori='". $ls_numdocori ."' AND codtiend='".$ls_codtie."' AND cod_pro='".$as_codproveedor."' ".
									"   AND CONCAT(promov,numdocori) NOT IN".
									"      (SELECT CONCAT(promov,numdocori) FROM sim_dt_movimiento".
									"        WHERE opeinv ='REV')".
									" ORDER BY nummov";
						...
						if($row1=$this->io_sql->fetch_row($li_exec1))
						{
							...
							if ($li_existencia > 0)
							{
								...
								if ($li_existencia < $li_canart)
								{
									...
									$lb_valido=$this->uf_sim_disminuir_articuloxmovimiento($as_codemp,$as_codart,$as_codalm,
									$as_nummov,$ls_numdocori,$li_existencia,$as_codproveedor,$ls_codtie,$aa_seguridad);
/*									
*/
								...
								{
									$lb_valido=$this->uf_sim_disminuir_articuloxmovimiento($as_codemp,$as_codart,$as_codalm,
									$ls_nummov,$ls_numdocori,$li_canart,$as_codproveedor,$ls_codtie,$aa_seguridad);
/*									
*/											if($lb_valido)
											{
												$lb_break=true;
					...
					if ($lb_valido)
					{
						...
						$lb_valido=$this->io_mov->uf_sim_insert_dt_movimiento($as_codemp,$as_nummov,$ad_fecdesaux,
																			  ...
																			  $as_codproveedor,$ls_codtie,$aa_seguridad);
				...
				else
				{
					...
					if($row=$this->io_sql->fetch_row($rs_metodo))
					{
						...
						$lb_valido=$this->io_mov->uf_sim_insert_dt_movimiento($as_codemp,$as_nummov,$ad_fecdesaux,
																			  $as_codart,$as_codalm,$ls_opeinv,$ls_codprodoc,
																			  $as_numsol,$ai_canart,$ai_preuniart,$ls_promov,
																			  $as_numorddes,$li_candesart,$ad_fecdesaux,$as_codproveedor,$ls_codtie,$aa_seguridad);
						...
						if($lb_valido)
						{
							...
							$lb_valido=$this->uf_sim_insert_dt_despacho($as_codemp,$as_numorddes,$as_codart,$li_i,$as_codalm,
																		$as_unidad,$ai_canorisolsep,$ai_canart,$ai_preuniart,
																		$ai_monsubart,$ai_montotart,$ai_orden,$ai_canpenart,$as_codproveedor,$ls_codtie,$aa_seguridad);
					    }
	...
	function uf_sim_disminuir_articuloxmovimiento($as_codemp,$as_codart,$as_codalm,$as_nummov,$ls_numdocori,$ai_cantidad,$as_codproveedor,$ls_codtie,$aa_seguridad)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->Despacho MÉTODO->uf_sim_disminuir_articuloxmovimiento ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
				$ls_evento="UPDATE";
				$ls_descripcion ="Actualizó existencia del articulo ". $as_codart.
								 " del almacen ".$as_codalm." Proveedor ".$as_codproveedor." asociado a la empresa ".$as_codemp." de la Tienda ".$ls_codtie." ".$ls_nomtie;
				$ls_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
												$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
												$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////
	...
	function uf_sim_update_sep($as_codemp,$as_numsol,$as_estsep)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->Despacho MÉTODO->uf_sim_update_sep ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_sim_obtener_dt_despacho($as_codemp,$as_numodrdes,&$ai_totrows,&$ao_object)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->despacho MÉTODO->uf_sim_obtener_dt_despacho ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
	...
	function uf_sim_obtener_dt_scg($as_codemp,$as_numodrdes,&$ai_totrows,&$ao_object)
	{
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->despacho MÉTODO->uf_sim_obtener_dt_scg ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			while($row=$this->io_sql->fetch_row($li_exec))
	...
	function uf_sim_load_contabilizacion($as_codemp,&$li_value)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->Despacho MÉTODO->uf_sim_load_contabilizacion ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
