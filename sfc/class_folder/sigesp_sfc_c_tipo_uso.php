...
function sigesp_sfc_c_tipo_uso()
{
	...
	function uf_select_tipo_rubro($ls_codtipo)
	{
		...
		$ls_cadena="SELECT * FROM sfc_tipouso
	...
	function uf_guardar_tipo_rubro($ls_codtipo,$ls_nomtipo,$aa_seguridad)
	{
		...
		if(!$lb_existe)
		{
            $ls_cadena= " INSERT INTO sfc_tipouso(dentipouso,codemp)
			              VALUES (UPPER('".$ls_nomtipo."'),'".$ls_codemp."') ";
			$this->io_msgc="Registro Incluido!!!";
			$ls_evento="INSERT";
		}
		else
		{
			$ls_cadena= "UPDATE sfc_tipouso
			             SET dentipouso=UPPER('".$ls_nomtipo."'), codemp='".$ls_codemp."' WHERE id_tipouso='".$ls_codtipo."'";
			//print $ls_cadena;
		...
		else
		{
			if($li_numrows>0)
			{
				...
				if($ls_evento=="INSERT")
				{
					/////////////////////////////////         SEGURIDAD               /////////////////////////////
					$ls_evento="INSERT";
					$ls_descripcion ="Insert� el Tipo de Uso ".$ls_nomtipo." Asociado a la Empresa ".$ls_codemp;
				...
				else
				{
					/////////////////////////////////         SEGURIDAD               /////////////////////////////
					$ls_evento="UPDATE";
					$ls_descripcion ="Actualiz� el Tipo de Uso ".$ls_nomtipo." Asociado a la Empresa ".$ls_codemp;
					$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
													$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
													$aa_seguridad["ventanas"],$ls_descripcion);
					/////////////////////////////////         SEGURIDAD               /////////////////////////////
	...
	function uf_delete_tipo_rubro($ls_codtipo,$aa_seguridad)
	{
		...
		if($lb_existe)
		{
		    	$ls_cadena= " DELETE FROM sfc_tipouso
							  WHERE id_tipouso='".$ls_codtipo."'";
				...
				else
				{
					if($li_numrows>0)
					{
						...
						////////////////////////////////         SEGURIDAD               /////////////////////////////
						$ls_evento="DELETE";
						$ls_descripcion ="Elimin�el Tipo de Uso ".$ls_codtipo." Asociado a la Empresa ".$ls_codemp;
						$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
														$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
														$aa_seguridad["ventanas"],$ls_descripcion);
						////////////////////////////////         SEGURIDAD               ////////////////////////////
