...
class sigesp_sim_c_revdespacho
{
	...
	function uf_sim_select_despacho($as_codemp,$as_numorddes)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->revdespacho MÉTODO->uf_sim_select_despacho ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
	...
	function uf_sim_select_dt_despacho($as_codemp,$as_numorddes,&$rs_data)
	{
		...
		//                 del cual fueron despachados los artículos, en la tabla de  sim_dt_despacho
		//	   Creado Por: Ing. Luis Anibal Lang
		// Fecha Creación: 10/02/2006							Fecha Última Modificación : 10/02/2006
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->revdespacho MÉTODO->uf_sim_select_dt_despacho ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_sim_obtener_despacho(&$ai_totrows,&$ao_object)
	{
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->revdespacho MÉTODO->uf_sim_obtener_despacho ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			$li_i=0;
	...
	function uf_sim_update_articulos($as_codemp,$as_numorddes,$aa_seguridad)
	{
		...
		{
			$lb_valido=false;
			$this->io_msg->message("CLASE->revdespacho MÉTODO->uf_sim_update_articulos ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			while($row=$this->io_sql->fetch_row($rs_data))
	...
	function uf_sim_update_status_despacho($as_codemp,$as_numorddes,$as_numsol,$aa_seguridad) 
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->revdespacho MÉTODO->uf_sim_update_status_despacho ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_sim_update_ordencompra($as_codemp,$as_numordcom) 
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->revdespacho MÉTODO->uf_sim_update_ordencompra ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
...
*/					if($lb_valido==false)
	...
	function uf_sim_update_sep($as_codemp,$as_numsol) 
	{
		...
		if($lb_valido)
		{
			...
			if($li_row===false)
			{
				$this->io_msg->message("CLASE->revdespacho MÉTODO->uf_sim_update_sep ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				$lb_valido=false;
			}
			else
			{
	...
	function uf_sim_select_estatus_sep($as_codemp,$as_numsol,&$as_estsep)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->revdespacho MÉTODO->uf_sim_select_estatus_sep ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function uf_sim_select_dt_contable($as_codemp,$as_codcmp)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->revdespacho MÉTODO->uf_sim_select_dt_contable ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
		}
		else
		{
			if($row=$this->io_sql->fetch_row($rs_data))
	...
	function uf_sim_delete_dt_contable($as_codemp,$as_codcmp,$aa_seguridad)
	{
		...
		if($lb_existe)
		{
			...
			if($li_row===false)
			{
				$this->io_msg->message("CLASE->almacen MÉTODO->uf_sim_delete_almacen ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
				$lb_valido=false;
			}
			else
			{
