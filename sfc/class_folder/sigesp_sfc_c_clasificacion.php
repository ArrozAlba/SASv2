...
function sigesp_sfc_c_clasificacion()
{
		...
		else
		{
			if($li_numrows>0)
			{
				...
				if($ls_evento=="INSERT")
				{
					/////////////////////////////////         SEGURIDAD               /////////////////////////////
					$ls_evento="INSERT";
					$ls_descripcion ="Inserto la Clasificacion ".$ls_codcla." Asociado a la Empresa ".$ls_codemp;
				...
				else
				{
					/////////////////////////////////         SEGURIDAD               /////////////////////////////
					$ls_evento="UPDATE";
					$ls_descripcion ="Actualizo la Clasificacion ".$ls_codcla." Asociado a la Empresa ".$ls_codemp;
					$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
													$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
													$aa_seguridad["ventanas"],$ls_descripcion);
					/////////////////////////////////         SEGURIDAD               /////////////////////////////
	...
	function uf_delete_clasificacion($ls_codcla,$aa_seguridad)
	{
		...
		if($lb_existe)
		{
			$ls_cadena = "DELETE FROM sfc_subclasificacion WHERE codcla='".$ls_codcla."'; ";
			...
			else
			{
				if($li_numrows>0)
				{
					...
					////////////////////////////////         SEGURIDAD               /////////////////////////////
					$ls_evento="DELETE";
					$ls_descripcion ="Elimino la Clasificacion ".$ls_codcla." Asociado a la Empresa ".$ls_codemp;
					$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
													$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
													$aa_seguridad["ventanas"],$ls_descripcion);
					/////////////////////////////////         SEGURIDAD               ///////////////////////////
	...
	function uf_guardar_lineassublineas($ls_codcla,$ls_codsub,$ls_densub,$aa_seguridad)
    {
		...
		else
		{
			if($li_row>0)
			{
			    /************    SEGURIDAD    **************/
				  $ls_evento="INSERT";
				  $ls_descripcion ="Inserto la Subclasificacion ".$ls_codsub.", Detalle de la Clasificacion ".$ls_codcla." Asociado a la Empresa ".$ls_codemp;
...
/*****************************************************************************************/
  ...
  /*****************************************************************************************/
	function uf_delete_sublineas($ls_codcla,$aa_seguridad)

	{
		...
		else
		{
			/*************    SEGURIDAD    **************/
			$ls_evento="DELETE";
			$ls_descripcion ="Elimino la Subclasificacion de la Clasificacion ".$ls_codcla." Asociado a la Empresa ".$ls_codemp;
			$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],$aa_seguridad["ventanas"],$ls_descripcion);
			/********************************************/
			$lb_valido=true;
			$this->io_sql->commit();
  ...
  /*****************************************************************************************/
	function uf_delete_lineassublineas($ls_codcla,$ls_codsub,$aa_seguridad)

	{
		...
		if($li_row===false)
		{
			...
			print"Error en metodo uf_delete_lineassublineas ".$this->io_function->uf_convertirmsg($this->io_sql->message);
		}
		else
		{
			/*************    SEGURIDAD    **************/
			$ls_evento="DELETE";
			$ls_descripcion ="Elimino la sublinea ".$ls_codsub.",Detalle de la Linea ".$ls_codcla." Asociado a la Empresa ".$ls_codemp;
			$lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],$aa_seguridad["ventanas"],$ls_descripcion);
			/********************************************/
			$lb_valido=true;
			$this->io_sql->commit();
	...
	function uf_update_sublineas($ls_codcla,$ls_codsub,$ls_densub,$aa_seguridad)
	 {
		...
		/***********************************************************************************************************/
		$lb_valido=false;
			$this->is_msgc="Error en metodo uf_update_sublineas".$this->io_funcion->uf_convertirmsg($this->io_sql->message);
			$this->io_sql->rollback();
			print $this->io_msgc;
			print $this->io_funcion->uf_convertirmsg($this->io_sql->message);
		/************************************************************************************************************/
		...
		else
		{
			if($li_row>0)
			{
				/*************    SEGURIDAD    **************/
				$ls_evento="UPDATE";
				$ls_descripcion ="Actualizó la subclasificacion ".$ls_codsub.", Detalle de la Clasificacion ".$ls_codcla." Asociado a la Empresa ".$ls_codemp;
				   $lb_variable= $this->seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],$aa_seguridad["ventanas"],$ls_descripcion);
				/**********************************************/
				$this->io_sql->commit();
				$lb_valido=true;
  ...
  /*Revisar, este llama a los metodos anteriores*/
...

	{
		...
		for ($li_i=1;$li_i<=$li_totalnuevas;$li_i++)
		{
			...
			for ($li_j=1;$li_j<=$li_totalfilasviejas;$li_j++)
			{
				...
				//if ($la_detallesviejos["codemp"][$li_j]==$ls_codemp && $la_detallesviejos["codcli"][$li_j]==$ls_codcli)
				{
					...
					if($la_detallesviejos["descsub"][$li_j] != $aa_detallesnuevos["descsub"][$li_i])
					{
					  $lb_update=true;
					}
					$lb_existe = true;
		...
		for ($li_j=1;$li_j<=$li_totalfilasviejas;$li_j++)
		{
			...
			if (!$lb_existe)
			{
				$this->uf_delete_lineassublineas($ls_codcla,$la_detallesviejos["cod_sub"][$li_j],$aa_seguridad);
			}
		}
	}
}
