...
if(array_key_exists("operacion",$_POST))
{
	...
	if(array_key_exists("combo_entidad",$_POST))
	{
			...
			if($lb_valest)
			 {
			  ...
			  <select name="combo_entidad" size="1" id="combo_entidad">
			   <option value="" >Seleccione...</option>
			 <?Php	
					...
					for($li_i=1;$li_i<=$li_totalfilas;$li_i++)
					{
						 ...
						 if ($ls_codigob==$ls_codent )
						 {						 	
							  print "<option  value='$ls_codigob' selected>$ls_noment</option>";
      ...
      $ld_monB=$io_funsob->uf_convertir_cadenanumero($ls_montot);
		   ...
		   if(($ls_espec==0)&&($ls_metforpago=="C"))
			...
			//alert (ls_codent);
			 ...
			 opener.ue_cargarfpago(ls_numinst,ld_monto,ls_denforpago,ls_nombanco,ls_codban,ls_codforpag,ls_metforpago,ls_codent);
			 close();
			</script>
	...
	with(f)
		{
			...
			if (ue_valida_null(txtmonto,"Monto")==false)
			 ...
			 else if (txtnuminst.value!="000000000000000" && txtmetforpago.value!="D" && txtmetforpago.value!="B")
			 {
			    if (txtnuminst.value=="")
				 {
