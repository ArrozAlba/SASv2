<?php
	...
	function uf_print_encabezado_pagina($ad_fechaanula,&$io_pdf)
	{
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$ls_titulo= utf8_decode("<b>COMPROBANTE DE ANULACIÓN DE COBRO</b>");
		...
		$io_pdf->addText(90,122,7,"Analista de Cobranza:"); // Agregar el título
		$io_pdf->addText(240,122,7,utf8_decode("Coordinador de Cobranza:")); // Agregar el título
	...
	function uf_print_cabecera($as_numdoc,$ad_fecmov,$ad_fechaanula,$as_nomproben,$as_conanu,$adec_total,&$io_pdf)
	{
		...
		$la_columna=array('numdoc'=>'COBRO','fecmov'=>utf8_decode('EMISIÓN'),'monto'=>'MONTO');
...
/*		$la_data=array(array('ordenes'=>'<b>Orden(es) de Pago(s):</b> '.$ls_solicitudes),
	...
	function uf_print_detalle($numfac,&$io_pdf)
	{
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                SELECT
                    ...
                    END as tipcancel,
                    moncancel,
                    (montoxcancel-moncancel) as montoxcancel
                FROM
                    public.sfc_dt_cobrocliente
                WHERE
                        numcob = '$numfac'
                ";
                $rs = pg_query($sql);
                ...
                //print_r($la_data_nm);
                    ...
                    while($la_data_nm = pg_fetch_array($rs))
                    {
                                            ...
                                            $ls_numfac=     $la_data_nm['numfac'];
                                            $ls_tipcancel=     $la_data_nm['tipcancel'];
                                            $ls_monto_cobro = $la_data_nm['moncancel'];;
                                            $ls_montoxcancel=     $la_data_nm['montoxcancel'];

                                            $ls_monto_cobro=number_format($ls_monto_cobro,2,",",".");
                                            $ls_montoxcancel=number_format($ls_montoxcancel,2,",",".");

                                            $la_data[$li_s]=array('numfac'=>$ls_numfac,'tipcancel'=>$ls_tipcancel,'moncancel'=>$ls_monto_cobro,'montoxcancel'=>$ls_montoxcancel);
                                            $li_s++;
                                            //print_r($la_data);
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'cols'=>array('estpro'=>array('justification'=>'center','width'=>195),
			 						   'numfac'=>array('justification'=>'center','width'=>180),
									   'tipcancel'=>array('justification'=>'center','width'=>80),
                                                                           'moncancel'=>array('justification'=>'center','width'=>80),
						 			   'montoxcancel'=>array('justification'=>'center','width'=>80))); // Justificación y ancho de la columna
		$la_columnas=array('numfac'=>utf8_decode('<b>Numero de Factura:</b>'),
						   'tipcancel'=>utf8_decode('<b>Tipo:</b>'),
                                                   'moncancel'=>utf8_decode('<b>Monto Cancelado:</b>'),
						   'montoxcancel'=>'<b>Monto Pendiente:</b>');
	...
	$ls_numcob=$_GET["numcob"];
        ...
        SELECT
            ...
            cobcli.totalret,
            cobcli.obsanu,
            to_char(cobcli.fecanu,'DD/MM/YYYY') as fecanu
            ,cli.razcli
            ,cli.cedcli
        FROM
	...
	$thisPageNum=$io_pdf->ezPageCount;
		...
		uf_print_encabezado_pagina($fechaAnulacion,&$io_pdf);
                uf_print_detalle($ls_numcob,&$io_pdf);
		uf_print_cabecera($ls_numcob,$fechaEmision,$fechaAnulacion,$nombreCliente,$obsAnulacion,$monto,$io_pdf); // Imprimimos la cabecera del registro
