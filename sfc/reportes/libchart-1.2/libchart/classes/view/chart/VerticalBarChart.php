<?php
	...
	class VerticalBarChart extends BarChart {
		 ...
		 */
		public function VerticalBarChart($width, $height) {
			parent::BarChart($width, $height);

			$this->emptyToFullRatio = 1 / 5;
			$this->plot->setGraphPadding(new Padding(15,25,80, 50));
		}
		...
		protected function printAxis() {
			...
			for ($i = 0; $i <= $pointCount; $i++) {
				...
				if ($i < $pointCount) {
					...
					next($pointList);	
					$label = $point->getX();
					$text->printDiagonal($img, $x + $columnWidth * 1 / 2, $graphArea->y2 + 5, $this->plot->getTextColor(), $label);
				}
			}
		}
		...
		protected function printBar() {
			...
			for ($j = 0; $j < $serieCount; $j++) {
				...
				for ($i = 0; $i < $pointCount; $i++) {
					...
					// Text
					$text->printCentered2($img,25, $this->plot->getTextColor(),"Unidades",$text->fontCondensed);
