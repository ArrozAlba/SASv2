...
if ($la_producto)
{
					...
					if($row2=$io_sql->fetch_row($rs_datauni10))
 				  {
					...
					for($li_i=1;$li_i<=$totrow2;$li_i++)
					{
						...
						" AND p.codart=a.codart AND a.codcla=cla.codcla AND t.dentipouso='".$ls_den."' ".
					...
					" AND tu.dentipouso='".$ls_den."'  AND sub.den_sub='".$ls_den_sub."' AND cla.codcla=p.codcla AND sub.codcla=cla.codcla AND f.numfac=df.numfac ".
					...
					" GROUP BY sub.cod_sub,sub.den_sub;";*/
						...
						$ls_cadena="SELECT sub.cod_sub,max(df.porimp),sub.den_sub,sum(df.canpro) as tcant, sum(((df.canpro*df.prepro)*(df.porimp/100))+" .
							...
							" AND tu.dentipouso='".$ls_den."' AND sub.den_sub='".$ls_den_sub."' AND cla.codcla=a.codcla AND sub.codcla=cla.codcla " .
							" AND f.numfac=df.numfac AND u.id_tipouso=tu.id_tipouso AND u.id_uso=a.id_uso AND p.codart=a.codart AND p.codemp=a.codemp" .
							" GROUP BY sub.cod_sub,sub.den_sub";
