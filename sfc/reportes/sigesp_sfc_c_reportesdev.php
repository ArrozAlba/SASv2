...
class sigesp_sfc_c_reportesdev
{
//ATRIBUTOS
  var $io_pdf;
  var $io_funcion;
...
///////////////////////////////////////////////////////////////////////////////////////////
  function colocar_cabecera($titulo,$orientacion)
  {
    ...
    //$cabecera = $this->io_pdf->openObject();
	  ...
	  {
      $this->io_pdf->add_imagen('imagenes/logo.jpeg',-5,-30,220);
	  }else
	  {
	    $this->io_pdf->add_imagen('imagenes/logo.jpeg',-10,-30,180);
	  }
...
function uf_calcular_montocobrado($ls_numfac)
{
	...
	$ls_cadena="SELECT numnot FROM sfc_nota WHERE nro_factura like '%".$ls_numfac."%' AND estnota='P' AND tipnot='CXC';";

	$li_numrows=$this->io_sql->select($ls_cadena);
	if($row=$this->io_sql->fetch_row($li_numrows))
	{
		$ls_cadenacobrado="SELECT sum(moncob) AS total FROM sfc_cobro WHERE numcarta='".$row["numnot"]."' AND estcob='C';";
	...
	function uf_calcular_montocobradofac($ls_numfac,$ls_monto)
{
	...
	$ls_cadena="SELECT numnot FROM sfc_nota WHERE nro_factura like '%".$ls_numfac."%' AND estnota='P' AND tipnot='CXC' and monto='".$ls_monto."';";
//print $ls_cadena;
	$li_numrows=$this->io_sql->select($ls_cadena);
	if($row=$this->io_sql->fetch_row($li_numrows))
	{
