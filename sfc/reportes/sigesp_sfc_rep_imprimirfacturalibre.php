...
$ls_forpago="";
//$ls_numcontrol=$_GET["ls_numcontrol"];
            $ls_sql=$_GET["sql"];
		...
		else
		{
		  ...
		  $la_formapago=$io_sql->obtener_datos($rs_datapago);
			...
			{										
				...
				elseif(stristr($ls_forpago2,"Ticket de Alimentaci�n")==TRUE){				 	
				    $ls_num=$la_formapago["numinst"][$f+1];
				    $ls_monto=$la_formapago["monto"][$f+1];
				    $ls_monto=number_format($ls_monto,2, ',', '.');
					$la_datospago[$f]["tipo"]="<b>$ls_forpago2 </b>";
...
// $reporte->add_titulo("left",$posx+5,8,"Observaciones:   ".$la_facturacion["obsfac"][1]);			
				...
				$reporte->add_titulo(103,12,12,"<b>FACTURA:  </b>  ".$ls_numfac);
				$reporte->add_titulo(73,18,8,"<b>LUGAR DE EMISION:</b>    ".$la_facturacion["dirtie"][1]);
		 	//	$reporte->add_titulo("left",22,9,"NOMBRE Y APELLIDO O RAZON SOCIAL:    ".$la_facturacion["razcli"][1]);
				...
				$io_pdf->ezSetY(695);
				$la_anchos = array(190);
				$la_datosbasicos[0]["contenido"]="<b>NOMBRE Y APELLIDO O RAZON SOCIAL:</b>    ".$la_facturacion["razcli"][1];
				$la_datosbasicos[1]["contenido"]="<b>DOMICILIO FISCAL: </b>  ".$la_facturacion["dircli"][1];
				...
				$reporte->add_titulo("left",$li_alto_usado+1,9,"</b><b>TELEFONO:</b>".$la_facturacion["telcli"][1]."  ".$la_facturacion["celcli"][1]);
				$reporte->add_titulo(73,$li_alto_usado+1,9,"</b><b>RIF. o C.I.: </b>    ".$la_facturacion["cedcli"][1]);
				//$reporte->add_titulo("left",151,9,"OBSERVACIONES:     ".$la_facturacion["obsfac"][1]);
				//$li_alto_usado=$io_pdf->get_alto_usado();
				$reporte->add_titulo("left",$li_alto_usado+6,9,"</b><b>ORDEN DE ENTREGA:</b> ".$la_facturacion["numordent"][1]);
				...
				if ($ls_conpago=='CREDITO')
				 {
				   $reporte->add_titulo(73,$li_alto_usado+11,9,"</b><b>CANTIDAD DE DIAS:</b> " .$la_facturacion["numdiacre"][1]);
				 }                         
				 unset($la_datosbasicos,$la_opcion);
				 $io_pdf->ezSetY(600);
				$la_anchos = array(193);
				$la_datosdetalles[0]["contenido"]="<b>DETALLES DE FACTURACI�N </b>  ";
			 ...
			 /*   $reporte->add_titulo(10,46,7,"CANTIDAD");
			   ...
			   // ----  Detalles de la FACTURA  -------
                ...
                $li_cuotas=count($la_facturacion["canpro"],COUNT_RECURSIVE);
				...
				{
 				 $la_datos[$i]["CANT"]= number_format($la_facturacion["canpro"][$i+1],2, ',', '.');
				 $ls_porimp= number_format($la_facturacion["porimp"][$i+1],2,',','.');

				 $la_datos[$i]["DESCRIPCION"]= "   ".$la_facturacion["denpro"][$i+1]." ".$la_facturacion["denunimed"][$i+1];
			    ...
			    $la_datos[$i]["PRECIO UNITARIO"]= number_format(rtrim($la_facturacion["prepro"][$i+1]),2,',','.');
		  	    $la_datos[$i]["TOTAL"]= number_format($la_facturacion["prepro"][$i+1]*$la_facturacion["canpro"][$i+1],2, ',', '.');

			    $subtotal= $subtotal+($la_facturacion["prepro"][$i+1]*$la_facturacion["canpro"][$i+1]);
				
				
				
				$la_iva=$la_iva+(round((($la_facturacion["porimp"][$i+1]/100)*$la_facturacion["prepro"][$i+1]),2)*$la_facturacion["canpro"][$i+1]);

                if ($la_facturacion["porimp"][$i+1]=='12')
					 {
					 $la_iva1=$la_iva1+(round((($la_facturacion["porimp"][$i+1]/100)*$la_facturacion["prepro"][$i+1]),2)*$la_facturacion["canpro"][$i+1]);					   		
				  ...
				  if ($la_facturacion["porimp"][$i+1]=='8')
					 ...
					 //$la_iva2= round($la_iva2,2)+ (round((round(($la_facturacion["porimp"][$i+1]/100),2)* round($la_facturacion["prepro"][$i+1],2)),2)*$la_facturacion["canpro"][$i+1]);
                                         $la_iva2=$la_iva2+(round((($la_facturacion["porimp"][$i+1]/100)*$la_facturacion["prepro"][$i+1]),2)*$la_facturacion["canpro"][$i+1]);
				...
				//$ls_base2=number_format($ls_base2,2, ',', '.');
                                $la_iva1=$_GET["moniva"];
                                $la_iva2=$_GET["monivas"];
                                $ls_base1=$_GET["baseiva1"];
                                $ls_base2=$_GET["baseiva2"];
				$la_total=$_GET["total"];
