...
if (!array_key_exists("la_logusr",$_SESSION))
	...
	// para crear el libro excel
		...
		$lo_archivo = tempnam("/tmp", "libroventa.xls");
	...
	//Parámetros para Filtar el Reporte
		...
		//print $as_fechadesde."--".$as_fechahasta;
		$ls_rango    = "MES: ".$ls_mes."    "."AÑO:".$ls_ano."";
	...
	//---------------------------------------------------------------------------------------------------------------------------
	//Busqueda de la data 
	/*$lb_valido=uf_insert_seguridad("<b>Libro de Ventas en Excel</b>"); // Seguridad de Reporte
	if($lb_valido)
	{*/
	...
	// Impresión de la información encontrada en caso de que exista
		...
		else // Imprimimos el reporte
		{
			...
			$lo_hoja->write(5, 4, "N° Factura",$lo_titulo);
			$lo_hoja->write(5, 5, "N° Control",$lo_titulo);
			$lo_hoja->write(5, 6, "N° Nota de Débito",$lo_titulo);
			$lo_hoja->write(5, 7, "N° Nota de Crédito",$lo_titulo);
			
			$lo_hoja->set_column(8,8,13);
			$lo_hoja->write(5, 8, "Transacción",$lo_titulo);
			$lo_hoja->set_column(9,9,22);
			$lo_hoja->write(5, 9, "N° Fact. Afect.",$lo_titulo);
			...
			$lo_hoja->write(5, 15, "Base Imponible 9%",$lo_titulo);
			$lo_hoja->set_column(16,16,10);
			$lo_hoja->write(5, 16, "9%",$lo_titulo);
			$lo_hoja->set_column(17,17,20);
			$lo_hoja->write(5, 17, "Impuesto 9%",$lo_titulo);
...
$ls_cadena="Select dt.porimp,f.numfac,SUM(dt.canpro*dt.prepro) as baseimponible,SUM((dt.canpro*dt.prepro)*dt.porimp/100) as deduccion from sfc_factura f,sfc_detfactura dt where f.codemp='".$ls_codemp."' and f.codtiend='".$ls_codtie."' and dt.numfac='".$ls_numfac."' and f.numfac=dt.numfac and f.fecemi BETWEEN '".$as_fechadesde."' AND '".$as_fechahasta."' GROUP by f.numfac,dt.porimp,f.fecemi  ORDER BY f.fecemi ASC";			
		...
		//print $ld_sinderiva;
			...
			//print "TF-".$ld_totbasimp8f."-TB-";
				...
				$lo_hoja->write($li_row, 15, $ld_baseimp9, $lo_dataright);
				$lo_hoja->write($li_row, 16, $ls_porimp9 , $lo_datacenter);
				$lo_hoja->write($li_row, 17, $ld_deduccion9, $lo_dataright);
...
$ld_totbasimp9 = $ld_totbasimp9f - $ld_totbasimp9d;
		...
		$lo_hoja->write($li_row, 15, $ld_totbasimp9, $lo_total);
		$lo_hoja->write($li_row, 16, '', $lo_total);
		$lo_hoja->write($li_row, 17, $ld_totaldedu9, $lo_total);
		...
		$lo_hoja->write($li_row+5, 3, "Ventas gravables al 9,00%", $lo_resumen);
		$lo_hoja->write($li_row+6, 3, "IVA Cobrado al 9,0%", $lo_resumen);
		...
		$lo_hoja->write($li_row+5, 4, $ld_totbasimp9, $lo_dataright);
		$lo_hoja->write($li_row+6, 4, $ld_totaldedu9, $lo_dataright);
