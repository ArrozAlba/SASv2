        ...
        <td><span class="style6">
			...
			if($lb_valtie)
			 {
			   ...
			   $li_totalfilas=$io_datastore->getRowCount("codtie");
			 }
			 else
			   $li_totalfilas=0;
          ...
          <select name="cmbtienda" size="1" id="cmbtienda">
            ...
            <?Php
					for($li_i=1;$li_i<=$li_totalfilas;$li_i++)
					{
					 $ls_codigo=$io_datastore->getValue("codtie",$li_i);
...
if($ls_operacion=="BUSCAR")
{
 $ls_cadena=" SELECT * ".
			" FROM sfc_cliente ".
			" WHERE codest ilike '".$ls_codesta."'  AND razcli ilike '".$ls_razcli."'  AND codtie ilike '".$ls_codtien."'";
			...
			else
			{
				if($row=$io_sql->fetch_row($rs_datauni))
				{
					...
					print "<td><a href=javascript:ue_ordenar('codtie','BUSCAR');><font color=#FFFFFF>Código</font></a></td>";
					...
					for($z=1;$z<=$totrow;$z++)
					{
				        ...
				        print "<td><a href=\"javascript: aceptar('$codcli','$nomcli','$dircli','$telcli','$celcli','$codpai','$codest','$codmun','$codpar');\">".$codcli."</a></td>";
...
<script language="JavaScript">
  function aceptar(codcli,nomcli,dircli,telcli,celcli,codpai,codest,codmun,codpar)
  {
    opener.ue_cargarcliente(codcli,nomcli,dircli,telcli,celcli,codpai,codest,codmun,codpar);
	close();
  }
