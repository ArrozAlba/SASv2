    ...
    <input name="operacion" type="hidden" id="operacion"  value="<? print $ls_operacion?>">
	 ...
	 <tr>
	  ...
	  <br>
       ...
       <td width="56"><div align="right">Nuevo Numero Factura</div></td>
...
//var_dump($_POST);
	...
	$ls_nrocon=$io_secuencia->uf_crear_codigo("","",$ls_correlativoconaux);
	//print $ls_nrocon;
	}
	//print $ls_nrocon;
	?>
	...
	$ls_cadena=" SELECT numcon  FROM sfc_factura  WHERE numcon= '".$ls_nroconntrolaux."' ";
	//print $ls_cadena;
          	$lb_valido2=false;
			$rs_datauni=$io_sql->select($ls_cadena);
			if($rs_datauni==false&&($io_sql->message!=""))
  ...
  function ue_aceptar()
  {
   ...
   //  alert(controlvalido);
	...
	//alert(nro_valido==true && controlvalido==true);
		...
		if(confirm("�Est� seguro de que ese es el correlativo que corresponde a la Factura es?  "+numfac+" Y Numero de Control: "+numcontrol))
		{
			//alert ('paso');
			valido=true;
			valido2=true;
			if(correlativo=="")
			{
				correlativo=numfac.substr(10,15);
			}
			if(correlativocon=="")
			{
				correlativocon=numcontrol.substr(10,15);
			}
			opener.ue_cargarnrofactura(numfac,correlativo,numcontrol,correlativocon);
			close();
		}
		else
		{
	...
	else
	{
		if (nro_valido=="")
		{
			...
			f.action="sigesp_cat_editfacturacontrol.php";
		  	f.submit();
