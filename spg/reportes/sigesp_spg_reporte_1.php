	  ...
	  else
	  {
	  	  ...
	  	  while($row=$this->SQL->fetch_row($rs_mov_spg))
		  {
			  ...
			  $ls_ctaban=$row["ctaban"];
  			  $ls_provbenef=$this->uf_seek_provbenef($ls_codemp,$ls_procede,$ls_comprobante,$ldt_fecha,$ls_codban,$ls_ctaban);
			  ...
			  if ($ldt_fecha_movimiento < $adt_fecini )
			  {
			   ...
			   /*
			   print $ldt_fecha_movimiento."--1--".$adt_fecini."<br>";
			   print $ldt_fecha_movimiento."--2--".$adt_fecfin."<br>";
			   print $ls_spg_cuenta."--3--".$as_cuenta_from."<br>";
			   print $ls_spg_cuenta."--4--".$as_cuenta_to."<br>";
			   */ 
			      ...
			      ($ls_spg_cuenta>=$as_cuenta_from) and ($ls_spg_cuenta<=$as_cuenta_to))
			  {
				  ...
				  if ($lb_previo==true)
				  {
					 ...
					 $this->dts_reporte->insertRow("spg_cuenta",$ls_spg_cuenta);					 
					 $this->dts_reporte->insertRow("denominacion",$ls_denominacion);
					 $this->dts_reporte->insertRow("provbenef",$ls_provbenef);
    ...
    {////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 // Fecha Creación :    15/04/2006          Fecha última Modificacion :      Hora :
	  ...
	  $ls_sql=" SELECT CMP.descripcion as cmp_descripcion, CMP.fecha as fecha, MOV.*,CTA.denominacion".
			  ...
			  "    AND MOV.codestpro5 = CTA.codestpro5 ".
			  "    AND MOV.spg_cuenta = CTA.spg_cuenta ".
              " ORDER BY   MOV.codestpro1,MOV.codestpro2,MOV.spg_cuenta,MOV.comprobante ";
	 $rs_data=$this->SQL->select($ls_sql);
	 if($rs_data===false)
	 {
		  $lb_valido=false;
...
/********************************************************************************************************************************/	
  function uf_seek_provbenef($as_codemp,$as_procede,$as_comprobante,$adt_fecha,$as_codban,$as_ctaban)
  {  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 //	      Function :	uf_spg_reporte_select_max_codestpro3
	 //         Access :	private
	 //     Argumentos :    $as_codestpro3  // codigo de estructura programatica 3 (referencia)
	 ...
	 $ls_sql=" SELECT tipo_destino,cod_pro,ced_bene ".
             " FROM   sigesp_cmp ".
	 ...
	 else
	 {
		if($row=$this->SQL->fetch_row($rs_data))
		{
			   ...
			   $ls_proveedor   = $row["cod_pro"];
			   $ls_beneficiario= $row["ced_bene"];	
				   
			   if($ls_tipo=="P")
			   {
						$ls_sqlp=" SELECT nompro         ".
							 	 " FROM   rpc_proveedor  ".
								 " WHERE  (codemp='".$as_codemp."'AND cod_pro='".$ls_proveedor."')";   
								                     		
						$rs=$this->SQL->select($ls_sqlp);
						if($row=$this->SQL->fetch_row($rs))
						{
							$ls_provbenef=$row["nompro"];
						}
			   }
			   if($ls_tipo=="B")
			   {
					   $ls_sqlb=" SELECT nombene,apebene  ". 
							    " FROM   rpc_beneficiario ".
							    " WHERE  (codemp='".$as_codemp."' AND ced_bene='".$ls_beneficiario."')";       
							                    	
						$rs=$this->SQL->select($ls_sqlb);
						if($row=$this->SQL->fetch_row($rs))
						{
							$ls_nombre=$row["nombene"]; 
							$ls_apellido=$row["apebene"];  
							$ls_provbenef=$ls_nombre.",".$ls_apellido;
						}   
			   }		   
		}
		else
