...
//------------------------------------------------------------------------------------------------------------------------------
	...
	$lb_valido=$io_report->uf_spg_reporte_acumulado_cuentas2($ls_codestpro1,$ls_codestpro2,$ls_codestpro3,$ls_codestpro4,
	 ...
	 else // Imprimimos el reporte
	 {
		...
		$ls_mensaje2 = str_repeat(' ',60).$ls_titulo1.str_repeat(' ',70).date("d/m/Y").' '.date("h:i a");

		$io_tcpdf->SetHeaderData($_SESSION["ls_logo"],20, $ls_mensaje, $ls_mensaje2);
		$io_tcpdf->setHeaderFont(Array(PDF_FONT_NAME_MAIN, '', 12));
	...
	$pdf->writeHTML($html, true, 0, true, 0);
		*/
		...
		if($lb_valido2)
		{
			...
			$io_tcpdf->uf_print_detalle_acumulado2($la_data); // Imprimimos el detalle
