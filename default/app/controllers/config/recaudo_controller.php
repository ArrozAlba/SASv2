...
class RecaudoController extends BackendController {
    ...
    public function listar($order='order.nombre.asc', $page='pag.1') { 
        ...
        $recaudo = new Recaudo();        
        $this->profesiones = $recaudo->getListadoRecaudo($order, $page);
        $this->order = $order;        
    ...
    public function agregar() {
        ...
        if(Input::hasPost('recaudo')) {
            if(Recaudo::setRecaudo('create', Input::post('recaudo'), array('empresa_id'=>$empresa->id, 'ciudad'=>Input::post('ciudad')))) {
