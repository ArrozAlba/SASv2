...
class sigesp_sim_c_unidadmedida
{
	...
	function uf_sim_select_unidadmedida($as_codunimed)
	{
		...
		if($rs_data===false)
		{
			$this->io_msg->message("CLASE->unidadmedida MÉTODO->uf_sim_select_unidadmedida ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
		}
		else
		{
	...
	function  uf_sim_insert_unidadmedida($as_codunimed,$as_denunimed,$as_unidad,$as_obsunimed,$aa_seguridad)
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->unidadmedida MÉTODO->uf_sim_insert_unidadmedida ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
	...
	function uf_sim_update_unidadmedida($as_codunimed,$as_denunimed,$as_unidad,$as_obsunimed,$aa_seguridad) 
	{
		...
		if($li_row===false)
		{
			$this->io_msg->message("CLASE->unidadmedida MÉTODO->uf_sim_update_unidadmedida ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message));
			$lb_valido=false;
			$this->io_sql->rollback();
		}
		else
