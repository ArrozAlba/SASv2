...
<br>
  ...
  <?php
	...
	{				
		$ls_sql="SELECT sim_toma.numtom,sim_toma.codalm,sim_toma.fectom,sim_toma.estpro, sim_dt_toma.cod_pro, sim_toma.obstom,
				       (SELECT nomfisalm FROM sim_almacen WHERE sim_toma.codalm=sim_almacen.codalm) AS nomfisalm 
				FROM sim_toma ,sim_dt_toma
				WHERE sim_toma.codemp=sim_dt_toma.codemp
				AND   sim_toma.codtiend=sim_dt_toma.codtiend
				AND   sim_toma.numtom=sim_dt_toma.numtom
				AND   sim_toma.codalm=sim_dt_toma.codalm
				AND   sim_toma.codemp = '".$ls_codemp."' 
				AND   sim_toma.codalm LIKE '".$ls_codalm."' 
				AND   sim_toma.numtom LIKE '".$ls_numtom."' 
				GROUP BY sim_toma.numtom,sim_toma.codalm,sim_toma.fectom,sim_toma.estpro, sim_dt_toma.cod_pro, sim_toma.obstom,nomfisalm
				ORDER BY sim_toma.numtom";
		//print $ls_sql;
		...
		if($row=$io_sql->fetch_row($rs_cta))
		{
			...
			for($z=1;$z<=$totrow;$z++)
			{
				...
				//print "<td><a href=\"javascript: aceptar('$ls_numtom','$ls_codalm','$ls_nomfisalm','$ld_fectom','$ls_estpro','$ls_codpro','$ls_obstom');\">".$ls_numtom."</a></td>";
				print "<td><a href=\"javascript: aceptar('$ls_numtom','$ls_codalm','$ls_nomfisalm','$ld_fectom','$ls_estpro','$ls_obstom','$ls_codpro');\">".$ls_numtom."</a></td>";
...
<script language="JavaScript">
	function aceptar(ls_numtom,ls_codalm,ls_nomfisalm,ld_fectom,ls_estpro,ls_obstom,ls_codpro)
	{
	...
	function ue_search()
  	{
		...
		f.action="sigesp_sim_cat_tomaalmacen.php";
		f.submit();
	}
</script>
</html>
