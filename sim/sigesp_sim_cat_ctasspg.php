...
if($ls_operacion=="BUSCAR")
{
   ...
   {			  
		$ls_sql = " SELECT DISTINCT spg_cuentas.spg_cuenta, spg_cuentas.denominacion as denspg,spg_cuentas.status 
					FROM   spg_cuentas, scg_cuentas 
					WHERE  spg_cuentas.codemp = '0001' 
					...
					AND    spg_cuentas.spg_cuenta ilike   '%".$ls_codigo."%'
					AND    spg_cuentas.denominacion ilike '%".$ls_denominacion."%' 
					AND    spg_cuentas.sc_cuenta ilike    '%".$ls_cuentaspg."%'    ";
		...
		for($li_i=0;$li_i<$li_total;$li_i++)
		{
			if($li_i==0)
			{
				$ls_sql=$ls_sql."   AND (spg_cuentas.spg_cuenta ilike '".$la_spg_cuenta[$li_i]."%'";
			}
			else
			{
				$ls_sql=$ls_sql."    OR spg_cuentas.spg_cuenta ilike '".$la_spg_cuenta[$li_i]."%'";
			}

		}

		$ls_sql= $ls_sql." ) GROUP BY spg_cuentas.spg_cuenta, spg_cuentas.denominacion, spg_cuentas.status 
...
<script language="JavaScript">
  ...
  function ue_search()
  {
	  ...
	  f.action="sigesp_sim_cat_ctasspg.php";
	  f.submit();
  }
</script>
</html>