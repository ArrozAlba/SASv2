...
<?php
	...
	{
		$ls_sql="SELECT sim_despacho.*, ".
				"      (SELECT denuniadm FROM spg_unidadadministrativa".
				"        WHERE spg_unidadadministrativa.coduniadm = sim_despacho.codunides) AS denunides,".
				"      (SELECT denuniadm FROM spg_unidadadministrativa".
				"        WHERE spg_unidadadministrativa.coduniadm = sim_despacho.coduniadm) AS denuniadm".
			    "  FROM	sim_despacho".
				" WHERE codemp = '".$ls_codemp."'".
				"   AND estrevdes = 1 AND coduniadm<>'' AND codunides<>''".
				"   AND numorddes iLIKE '".$ls_numorddes."'".
				"   AND numsol iLIKE '".$ls_numsol."'";
...
<script language="JavaScript">
	function aceptar(ls_numorddes,ls_numsol,ls_coduniadm,ls_denuniadm,ld_fecdes,ls_codunides,ls_denunides,ls_obsdes,ls_tiposal)
	{
		...
		if(ls_tiposal!='SAL'){
			...
			opener.document.form1.tipo.value="Salida";
			opener.document.form1.operacion.value="BUSCARDETSALIDA";
			opener.document.form1.txtcodalm.value=ls_codunides;
		}

		opener.document.form1.action="sigesp_sim_p_despacho.php";
