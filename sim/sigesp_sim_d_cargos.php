...
require_once("sigesp_sim_c_cargos.php");
$io_siv=new sigesp_sim_c_cargos();
...
<?php
	...
	{
		case "":
			$lb_valido=$io_siv->uf_sim_select_cargos($ls_codemp,$ls_codart,$li_totrows,$lo_object);		
			if (!$lb_valido)
			{
		...
		case "GUARDAR":
			...
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				...
				if(array_key_exists("chkagregar".$li_i,$_POST))
				{
					...
					if ($li_check==1)
					{
						$lb_encontrado=$io_siv->uf_sim_select_cargosxarticulo($ls_codemp,$ls_codart,$ls_codcar);
						if(!$lb_encontrado)
						{
							$lb_valido=$io_siv->uf_sim_insert_cargosxarticulo($ls_codemp,$ls_codart,$ls_codcar,$la_seguridad);
						}
					}// fin  if($li_check==1)
				...
				else
				{
					...
					if($lb_encontrado)
					{
						$lb_valido=$io_siv->uf_sim_delete_cargosxarticulo($ls_codemp,$ls_codart,$ls_codcar,$la_seguridad);
...
<script language="JavaScript">
	...
	function ue_guardar()
	{
		...
		f.action="sigesp_sim_d_cargos.php";
		f.submit();
	}
