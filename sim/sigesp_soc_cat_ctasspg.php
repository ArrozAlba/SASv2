...
if($ls_operacion=="BUSCAR")
{
   ...
   else
   {
	    $ls_sql = " SELECT  DISTINCT spg_cuenta, denominacion, codestpro1,codestpro2, codestpro3,sc_cuenta,status   ".
	              " FROM    spg_cuentas                                                                             ".
		          " WHERE   codemp = '".$as_codemp."'                   AND  spg_cuenta ilike '%".$ls_codigo."%' AND ".
				  "         denominacion ilike '%".$ls_denominacion."%'  AND  sc_cuenta  ilike '%".$ls_cuentaspg."%'  ";
		...
		for($li_i=0;$li_i<$li_total;$li_i++)
		{
			if($li_i==0)
			{
				$ls_sql=$ls_sql."   AND spg_cuenta ilike '".$la_spg_cuenta[$li_i]."%'";
			}
			else
			{
				$ls_sql=$ls_sql."    OR spg_cuenta ilike '".$la_spg_cuenta[$li_i]."%'";
			}
