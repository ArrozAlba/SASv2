<?php
	...
	function uf_print_detalle($la_data,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>1, // Mostrar encabezados
						 ...
						 'titleFontSize' => 8.5,  // Tama�o de Letras de los t�tulos
						 'showLines'=>1, // Mostrar L�neas
						 'shaded'=>0, // Sombra entre l�neas
						 'width'=>500, // Ancho de la tabla
						 'maxWidth'=>500, // Ancho M�ximo de la tabla
						 'xOrientation'=>'center', // Orientaci�n de la tabla
						 'cols'=>array('codart'=>array('justification'=>'left','width'=>105), // Justificaci�n y ancho de la columna
						 			   'denart'=>array('justification'=>'left','width'=>150), // Justificaci�n y ancho de la columna
						 			   'exiart'=>array('justification'=>'right','width'=>57), // Justificaci�n y ancho de la columna
						 			   'minart'=>array('justification'=>'right','width'=>57), // Justificaci�n y ancho de la columna
						 			   'reoart'=>array('justification'=>'right','width'=>57),
						 			   'tienda'=>array('justification'=>'center','width'=>74))); // Justificaci�n y ancho de la columna
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);
	...
	$ls_titulo="Listado de Articulos a Solicitar";
	if($ls_coddesde!="")
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	$lb_valido=$io_report->uf_select_articulossolicitar($ls_codemp,$ls_coddesde,$ls_codhasta,$li_ordenart,$ls_tienda_desde,$ls_tienda_hasta); // Cargar el DS con los datos de la cabecera del reporte
	...
	else // Imprimimos el reporte
	{
		...
		$tm=306-($li_tm/2);
		$io_pdf->addText($tm,490,10,$ls_fecha); // Agregar el t�tulo
		...
		for($li_i=1;$li_i<=$li_totrow;$li_i++)
		{
			...
			$li_exiart= $io_report->ds->data["existencia"][$li_i];
			$li_minart= $io_report->ds->data["minart"][$li_i];
			$li_reoart= $io_report->ds->data["reoart"][$li_i];
			$li_tienda= $io_report->ds->data["dentie"][$li_i];
			if(($li_exiart<=$li_minart)||($li_exiart<=$li_reoart))
			{
				...
				$la_data[$li_i]=array('codart'=>$ls_codart,'denart'=>$ls_denart,'exiart'=>$li_exiart,'minart'=>$li_minart,
									  'reoart'=>$li_reoart,'tienda'=>$li_tienda);
			}
		}
		if($li_aux>0)
		{
			...
			$la_datat[1]=array('total'=>'<b>Total de Articulos</b>','canart'=>$li_totart);
