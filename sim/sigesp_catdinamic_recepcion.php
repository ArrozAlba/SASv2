...
<br>
    <table width="500" border="0" cellpadding="1" cellspacing="1" class="formato-blanco" align="center">
      ...
      <tr>
        <td width="100"><div align="right">Numero </div></td>
        <td width="150" height="22"><div align="left">
      ...
      <tr>
	   ...
	   <tr>
        <td><div align="right">Fec. Desde</div></td>
        <td height="22"><div align="left">
          <input name="txtfdesde" type="text" id="txtfdesde" value="<? print $ls_fdesde ?>" size="12" maxlength="10" readonly="true" datepicker="true">
</div></td>
		<td><div align="right">Fec. Hasta</div></td>
        <td><div align="left">
          <input name="txtfhasta" type="text" id="txtfhasta" value="<? print $ls_fhasta ?>" size="12" maxlength="10" readonly="true" datepicker="true">
</div></td>
      </tr>
	   <tr>
        <td>&nbsp;</td>
...
<?php
	...
	if(array_key_exists("operacion",$_POST))
	{
		...
		$ls_fdesde=$io_func->uf_convertirdatetobd($_POST["txtfdesde"]);
		$ls_fhasta=$io_func->uf_convertirdatetobd($_POST["txtfhasta"]);
	}
	else
	{
		$ls_operacion="";
	...
	if($ls_operacion=="BUSCAR")
	{
		...
		if( ($ls_fdesde <> "") or ($ls_fhasta <> "")){
			$ls_fecha = " AND fecrec>='".$ls_fdesde."' AND fecrec<='".$ls_fhasta."' ";
		}else{
			$ls_fecha = " ";
		}
		$ls_sql="SELECT sim_recepcion.*, ".
				"      (SELECT nomfisalm FROM sim_almacen".
				"        WHERE sim_almacen.codalm = sim_recepcion.codalm) AS nomfisalm,".
				"      (SELECT nompro FROM rpc_proveedor".
				"        WHERE rpc_proveedor.cod_pro = sim_recepcion.cod_pro) AS nompro".
			    "  FROM sim_recepcion".
				" WHERE codemp = '".$ls_codemp."'".
				"   AND estrevrec = 1".
				"   AND numordcom ilike '".$ls_numordcom."'".
				"   AND codalm ilike '".$ls_codalm."'".$ls_fecha.
				" ORDER BY numordcom";
		//print $ls_sql."<br>";
...
<script language="JavaScript">
	function aceptar(ls_numconrec,ls_numordcom,ls_obsrec,ls_codpro,ls_nompro,ls_codalm,ls_nomfisalm,ls_estpro,ls_estrec,ld_fecrec)
	{
		...
		var tienda = ls_codalm;
		opener.document.form1.txtcodtiend.value=tienda.substring(6);
		...
		opener.document.form1.action="sigesp_sim_p_recepcion.php";
	...
	function ue_search()
  	{
		...
		if( (f.txtnumordcom.value == "") && (f.txtcodalm.value=="") ){
			if( (f.txtfdesde.value == "") || (f.txtfhasta.value == "")){
				alert ("Debe indicar el rango de fecha de la Recepci�n!!");
			} else{
